...
if(pageUrl.indexOf('http')>-1){
  this.JS_DIRECTRY = "https://imam-pc81.github.io/tag/tools/";
...
//jqueryを読み込んでから起動させたいので関数化。
function csvDataSet(){
  $.ajax({
    url:JS_DIRECTRY +"ajax.php",
    type:'GET',
    dataType:'jsonp',
    async:false,
    cache: false,
    data: {
      url: JS_DIRECTRY +"A/A.csv"
    },
    error:function() {
    ...
    success:function(data){
      ...
      csvA.sort(function(a, b) {return b.length - a.length;});
    }
  })
...
//var csvA = csv2Array(JS_DIRECTRY +"A/A.csv");
// チェックするダミーテキスト
ckDummys = new Array(
  '(ダミー)+',
  '(ﾀﾞﾐｰ)+',
...
function toolbar(){
  ...
  <ul>\
  <li><label><input type="radio" onclick="tool_ng();" name="tool">Check Summary</label></li>\
  <li><label><input type="radio" onclick="tool_check();" name="tool">Separate Tool</label></li>\
  <li><label><input type="radio" onclick="tool_extraction();" name="tool">Extract</label></li>\
  </ul>\
  </form>\
