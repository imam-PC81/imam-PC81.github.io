...
--------------------------------------------- */
function p_lang(){
  var name = 'Language Code';
  var description = 'Language code are specified correctly?';
  var value;
...
function p_charset(){
  var name = 'Charset';
  var description = 'Charset are specifieid correctly';
  var value;
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function p_LineFeedCode(){
  var name = 'LF Code';
  var description = 'LF Code are specified correctly';
  var value;
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function p_title(){
  var name = 'title';
  var description = '&lt;title&gt;tag has been set';
  var value;
...
function p_description(){
  ...
  var description = 'Met Description has been set';
  var value;
  var ng = 0; // NGフラグ 0:OK 1:NG 2:有 3:無
...
function p_keywords(){
  var name = 'keywords';
  var description = 'Keyword is set';
  var value;
  var ng = 0; // NGフラグ 0:OK 1:NG 2:有 3:無
...
function p_canonical(){
  var name = 'canonical';
  var description = 'Canonical is set, the value is correct or no';
  var value;
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function p_head1(html){
  var name = 'H1';
  var description = 'Did we have H1? and only 1?';
  var value = '';
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function p_head2(html){
  var name = 'H2';
  var description = 'Are you set the H2 tag?';
  var value = '';
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function ng_alt(){
  var name = 'Image';
  var description = 'Check ALT on IMG';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function ex_imgPass(){
  ...
  if(html.match(/<img[^>]*>/i)){
    ...
    jQuery('#debagBarInfoL #infoTableL th').each(function(){
      ...
      //changed to :
      jQuery(this).next('td').text(jQuery(this).children('img').attr('src'));
      i++;
    });
  }
...
function c_src(){
  ...
  // タグを順番にチェック
  for(var i=0; i<len; i++){
    var datT = String(datTags[i].getAttribute('src'));
    // 値抽出
    if (datT == ''){
      datT = 'src=""';
      ID = 'checkValue_src';
    } else if(datT == 'null'){
      datT = 'There is no src';
      ID = 'checkValue_src checkValue_ng2';
    }else{
      ID = 'checkValue_src';
    }
    var datV = datT;
...
function ex_src(){
  ...
  if(html.match(/<img[^>]*>/i)){
    ...
    // tr生成、thにimg出力
    jQuery('#debagBarInfoL #infoTableL').addClass('src');
    var len = imgs.length;
  ...
  // 一覧の画像クリックで該当に移動
  jQuery('#infoTableL.src th').click(function(){
    var imgNum = jQuery(this).children('img').attr('class').replace(/imgList/, '');
    var datTags = document.getElementsByTagName('img');
    var datE = jQuery(datTags[imgNum]);
...
function ng_src(){
  var name = 'Image';
  var description = 'Check SRC on IMG';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function ex_imgSize(){
  ...
  if(html.match(/<img[^>]*>/i)){
    ...
    jQuery('#debagBarInfoL #infoTableL th').each(function(){
      ...
      var wh = 'Defined　width：' + width + ' height：' + height + '<br>' + 'Real width：' + width_v + ' height：' + height_v;
...
function ng_dummyLink(){
  var name = 'Link';
  var description = 'Check Dummy link is not used (emarketing, web-works)';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
/* ---------------------------------------------
httpLink　NGワードチェック乃壱
--------------------------------------------- */
function c_testcheckA(){
  var ID = "checkValue_testcheckA";
...
//    ClearCheckValue(ID);
    ...
    $('body *').each(function(){
      ...
      if(!tag_id.match(/debag/i)){
        if(!$(this).parents('#debagBar')[0] && !$(this).parents('#debagBarSub')[0]){
          for (var u=0;u<csvA.length;u++) {
            if(csvA[u] != ""){
  ...
  $('body *').each(function(){
    ...
    if(!tag_id.match(/debag/i)){
      if(!$(this).parents('#debagBar')[0] && !$(this).parents('#debagBarSub')[0]){
        for (var u=0;u<csvA.length;u++) {
          if(csvA[u] != ""){
//            var re = new RegExp( csvA[u], "ig");
//            text = text.replace(re,"<span class='checkValue_checkA' style='color:red;'>"+csvA[u]+"</span>");
            var re = new RegExp( "(>[^<]*)("+csvA[u]+")([^>]*?<)", "ig");
            text = text.replace(re,"$1<span class='checkValue_checkA' style='color:red;'>$2</span>$3");
          }
        }
...
httpLink　NGチェック用乃壱
--------------------------------------------- */
function ng_testcheckA(){
  var name = 'Link';
  var description = 'Test server should not be used (emarketing, web-works)';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
/* ---------------------------------------------
httpLink　NGワードチェック乃弐
--------------------------------------------- */
function c_testcheckB(){
  var ID = "checkValue_testcheckB";
  ...
  for (var t=0;t<Tags.length;t++) {
    ...
    for(var i=0; i<len; i++){
      ...
      // 各URLを順番にチェック
      for (var u=0;u<csvB.length;u++) {
        var key = csvB[u];
        var re = new RegExp(key,"i");

        if(datT.match(re) && !datT.match(/googleadservices\.com/i) && !datT.match(/advg\.jp/i) && !datT.match(/c\.iogous\.com/i) && !datT.match(/VL\/Trace\?/i) && !datT.match(/g\.doubleclick\.net/i) && !datT.match(/club_t-www\.baynote\.net/i) && !datT.match(/trck\.dlpo\.jp/i)){
          ID = "checkValue_testcheckB checkValue_ng2";
          // 何番目のタグか確定
          var datN = datTags[i];
          // 該当箇所に表示
          insertHtml(datN,datT,ID);
...
httpLink　NGチェック用乃弐
--------------------------------------------- */
function ng_testcheckB(){
  var name = 'Link';
  var description = 'Uset test server path or not （emarketing,web-works）';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
/* ---------------------------------------------
httpLink　NGワードチェック乃参
--------------------------------------------- */
function c_testcheckC(){
  var ID = "checkValue_testcheckC";
  ...
  for (var t=0;t<Tags.length;t++) {
    ...
    for(var i=0; i<len; i++){
      ...
      // 各URLを順番にチェック
      for (var u=0;u<csvC.length;u++) {
        var key = csvC[u];
        var re = new RegExp(key,"i");

        if(datT.match(re) && !datT.match(/googleadservices\.com/i) && !datT.match(/advg\.jp/i) && !datT.match(/c\.iogous\.com/i) && !datT.match(/VL\/Trace\?/i) && !datT.match(/g\.doubleclick\.net/i) && !datT.match(/club_t-www\.baynote\.net/i) && !datT.match(/trck\.dlpo\.jp/i)){
          ID = "checkValue_testcheckC checkValue_ng2";
          // 何番目のタグか確定
          var datN = datTags[i];
          // 該当箇所に表示
          insertHtml(datN,datT,ID);
...
httpLink　NGチェック用乃参
--------------------------------------------- */
function ng_testcheckC(){
  var name = 'Link';
  var description = 'Using test server path or not（emarketing,web-works）';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function ng_httpsLink(){
  var name = 'Link';
  var description = 'Does HTTP files being read on HTTPS pages';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function c_link_title(){
  var ID = "checkValue_link_title";
  ...
  for (var t=0;t<Tags.length;t++) {
    ...
    // タグを順番にチェック
    for(var i=0; i<len; i++){
      var datT = String(datTags[i].getAttribute('title'));
      // 値抽出
      if (datT == ''){
        datT = 'title=""';
      } else if(datT == 'null'){
...
function ng_anchor(){
  var name = 'Link';
  var description = 'Anchor link check';
  var value = new Array();
...
/* my function, search CRLF inside P tag */
function ex_crlf() {
  toolClearInfoL();
  infoView();
  jQuery('#debagBarInfoL .infoHead p').text('CRLF inside P');

  $('p').each(function() {
          var text  = $(this).html();
          var data  = $(this).html().search(/[\n\r]/g);
          var rep   = $(this).html().replace(/[\n\r]/g, "<span class='ng'>(CRLF was here)</span><br>");
          if(data != -1) {
            // console.log(text)
            var td = '<tr><td>'+rep+'</td></tr>';
...
function ex_link2(){
  ...
  // 情報エリア表示
  infoView();
  jQuery('#debagBarInfoL .infoHead p').text('Link一覧');
  ...
  if(html.match(/<a [^>]*>|<area [^>]*>/i)){
    ...
    jQuery('#debagBarInfoL #infoTableL th').each(function(){
      ...
      if(jQuery(this).children('a').size() > 0){
        ...
        var title = jQuery(this).children('area').attr('title');
      }
      ...
      if(typeof title == 'undefined'){
        title = 'no title';
        jQuery(this).next('td').next('td').addClass('attrNG');
      }
      jQuery(this).next('td').text(href);
      jQuery(this).next('td').next('td').text(target);
      jQuery(this).next('td').next('td').text(title);
    });
  }
...
function ng_kana(){
  var name = 'Content';
  var description = 'No single-byte kana （ｰﾞﾟ｡｢｣､)';
  var value = new Array();
...
function ng_dummyText(){
  var name = 'Coontent';
  var description = 'No Dummy Text (dummy, test, text, yyyy.mm.dd)';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function ng_dummyFile(){
  var name = 'Coontent';
  var description = 'No dummy test (dummy, test, sample)';
  var value = new Array();
  var ng = 0; // NGフラグ 0:OK 1:NG
...
function ng_emptyTag(){
  var name = 'Source Check';
  var description = 'Check closing tag, in JS too';
  var value;
...
function t_adplan(html){
  var name = 'ADPLAN';
  var description = 'Correctly followed the instruction';
  var value = '';
  var ng = 0; // NGフラグ 0:OK 1:NG
